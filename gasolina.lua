...
local modules = {
    ...
    ["neverlose/inspect"] = function()
        local content = files.read("neverlose/inspect")
        return content
    end,

    ["neverlose/memory"] = function()
        local content = files.read("neverlose/memory.bin")
        -- 如果读到了字符串，尝试编译并执行它
        ...
        -- 读不到或者执行失败，就返回一个空壳模块，避免后面调用报错
        return {
            hook_func = function() end,
            unhook_func = function() end,
        }
    end,

    ["neverlose/easing"] = function()
        local content = files.read("neverlose/easing")
        return content
    end,

    ["neverlose/wapi"] = function(require)
